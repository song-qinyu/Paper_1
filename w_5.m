%% ============================================================
%  Pipeline Hybrid — Stage 竞选框架 v3（全15算法 + SHAP风格7图）
%
%  算法主体：用户最新代码（document 4，15算法原版）
%  可视化  ：SHAP风格 绿金双色系，共7张图自动保存PNG
%
%  Bug修复清单：
%    1. 删除 FontStyle（部分MATLAB版本text()不支持）
%    2. caxis/clim 双写兼容新旧版本
%    3. smooth 不可用时自动降级为线性拟合
%    4. rng_f/rng6 除零保护
%    5. safe_set_ax 改为普通函数，避免嵌套函数句柄问题
%% ============================================================
...
fprintf('ok fig1_stage_ranking.png\n');
...
fprintf('ok fig2_shap_beeswarm.png\n');
...
fprintf('ok fig3_main_vs_interaction.png\n');
...
fprintf('ok fig4_shap_heatmap.png\n');
...
fprintf('ok fig5_algo_interaction_matrix.png\n');

%% ===== 图6 =====
[~,top6_idx] = sort(mean(abs(shap_mat),2),'descend');
top6 = top6_idx(1:min(6,nAlgo));

fig6 = figure('Color',C_W,'Name','fig6','Position',[80 60 1300 820]);
for k=1:length(top6)
    ax = subplot(2,3,k);
    a_idx=top6(k);
    ...
    xlabel('适应度 (m)','FontSize',9); ylabel('SHAP value','FontSize',9);
    title(algos{a_idx},'FontSize',11,'FontWeight','bold','Color',[0.2 0.2 0.2]);
    set(ax,'Color',C_W,'Box','off','TickDir','out','FontSize',9,'GridAlpha',0.08,...
        'XColor',[0.35 0.35 0.35],'YColor',[0.35 0.35 0.35]);
    grid on;
end
...
fprintf('ok fig6_shap_dependence.png\n');
