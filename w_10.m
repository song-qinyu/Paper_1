...
%  + EWM-TOPSIS Multi-Criteria Evaluation (综合评价)
%
...
%
%  EWM-TOPSIS: 7 metrics -> objective weights -> C_i score
%  Metrics: Best, Mean, Std, Worst, Median, CV, Conv_Rate
%% ============================================================
clc; clear; close all; tic;

fprintf('========================================================\n');
fprintf('  Parallel Algorithm Benchmark — 15 Algorithms\n');
fprintf('  5 Categories | EWM-TOPSIS Multi-Criteria Ranking\n');
fprintf('========================================================\n\n');
...
%% ==================== 5. Aggregate ====================
best_overall = min(best_fit_mat, [], 2);
mean_overall = mean(best_fit_mat, 2);
std_overall  = std(best_fit_mat, 0, 2);
conv_mean    = squeeze(mean(conv_mat, 2));
...
%% ==================== 7. EWM-TOPSIS Evaluation ====================
% ── 构建决策矩阵 X (nAlgo × 7 metrics) ─────────────────────────
%
%  7 个评价指标：
%    M1: Best       (越小越好 → cost)
%    M2: Mean       (越小越好 → cost)
...
%    M4: Worst      (越小越好 → cost)
%    M5: Median     (越小越好 → cost)
%    M6: CV         (变异系数 = Std/Mean, 越小越好 → cost)
%    M7: Conv_Rate  (收敛速率 = 初始适应度-最终/初始, 越大越好 → benefit)
%
%  指标类型: cost型用 max-x 归一化, benefit型用 x-min 归一化

METRIC_NAMES = {'Best','Mean','Std','Worst','Median','CV','Conv\_Rate'};
METRIC_TYPE  = {'cost','cost','cost','cost','cost','cost','benefit'};
nMetric = length(METRIC_NAMES);

% 计算各指标值
worst_overall  = max(best_fit_mat, [], 2);
median_overall = median(best_fit_mat, 2);
cv_overall     = std_overall ./ (mean_overall + 1e-10);

% 收敛速率：(初始checkpoint适应度 - 最终checkpoint适应度) / 初始
conv_init  = conv_mean(:,1);
conv_final = conv_mean(:,end);
conv_rate  = (conv_init - conv_final) ./ (conv_init + 1e-10);

% 决策矩阵 X: nAlgo × nMetric
X_raw = [best_overall, mean_overall, std_overall, worst_overall, ...
         median_overall, cv_overall, conv_rate];

% ── Step 1: Min-Max 归一化 → r_ij ───────────────────────────
%  cost型:    r_ij = (max(xj) - xij) / (max(xj) - min(xj))
%  benefit型: r_ij = (xij - min(xj)) / (max(xj) - min(xj))
R = zeros(nAlgo, nMetric);
for j = 1:nMetric
    col = X_raw(:,j);
    mn  = min(col); mx = max(col);
    rng = mx - mn;
    if rng < 1e-12, rng = 1e-12; end   % 防止除零
    if strcmp(METRIC_TYPE{j}, 'cost')
        R(:,j) = (mx - col) / rng;
    else
        R(:,j) = (col - mn) / rng;
    end
...
% ── Step 2: EWM — 信息熵计算客观权重 w_j ───────────────────
%  p_ij = r_ij / sum_i(r_ij)
%  e_j  = -1/ln(n) * sum_i(p_ij * ln(p_ij))
%  w_j  = (1 - e_j) / sum_k(1 - e_k)

P = R ./ (sum(R, 1) + 1e-12);          % nAlgo × nMetric
% 处理 p=0 时 p*ln(p)=0
lnP = log(P + 1e-300);
lnP(P < 1e-300) = 0;

e_j = -1/log(nAlgo) * sum(P .* lnP, 1);  % 1 × nMetric
e_j = min(1, max(0, e_j));                % 裁剪到 [0,1]
w_j = (1 - e_j) / sum(1 - e_j);          % 归一化权重

fprintf('========================================================\n');
fprintf('  EWM Objective Weights\n');
fprintf('========================================================\n');
for j = 1:nMetric
    fprintf('  %-12s  e_j=%.4f   w_j=%.4f\n', ...
        strrep(METRIC_NAMES{j},'\_','_'), e_j(j), w_j(j));
end
fprintf('  Weight sum = %.6f\n', sum(w_j));
fprintf('========================================================\n\n');

% ── Step 3: 加权归一化矩阵 V = w_j * r_ij ──────────────────
V = R .* w_j;    % nAlgo × nMetric (broadcast)
...
% ── Step 5: 欧氏距离 D+, D- ─────────────────────────────────
D_pos = sqrt(sum((V - V_pos).^2, 2));   % nAlgo × 1
D_neg = sqrt(sum((V - V_neg).^2, 2));

% ── Step 6: 综合得分 C_i ────────────────────────────────────
C_score = D_neg ./ (D_pos + D_neg + 1e-12);   % 越大越优

% ── 排名 ────────────────────────────────────────────────────
[C_sorted, rank_idx] = sort(C_score, 'descend');
rank_names = ALGO_NAMES(rank_idx);
rank_cats  = algo_cat(rank_idx);

fprintf('========================================================\n');
fprintf('  TOPSIS Comprehensive Ranking  (C_i: higher = better)\n');
fprintf('========================================================\n');
fprintf('  %-4s %-8s %-22s %-8s %-8s %-8s\n', ...
    'Rank','Algo','Category','D+','D-','C_i');
fprintf('  %s\n', repmat('-',1,64));
for k = 1:nAlgo
    ai = rank_idx(k);
    fprintf('  %-4d %-8s %-22s %-8.4f %-8.4f %-8.4f\n', ...
        k, ALGO_NAMES{ai}, CAT_NAMES{algo_cat(ai)}, ...
        D_pos(ai), D_neg(ai), C_score(ai));
end
fprintf('  TOPSIS Champion: %s  (C_i = %.4f)\n', ...
    rank_names{1}, C_sorted(1));
fprintf('========================================================\n\n');

% 保存EWM-TOPSIS结果结构体
ewm_topsis.X_raw      = X_raw;
ewm_topsis.R          = R;
ewm_topsis.w_j        = w_j;
ewm_topsis.e_j        = e_j;
ewm_topsis.V          = V;
ewm_topsis.V_pos      = V_pos;
ewm_topsis.V_neg      = V_neg;
ewm_topsis.D_pos      = D_pos;
ewm_topsis.D_neg      = D_neg;
ewm_topsis.C_score    = C_score;
ewm_topsis.rank_idx   = rank_idx;
ewm_topsis.rank_names = rank_names;
...
% ── Fig 8 & 9: EWM-TOPSIS 专属可视化 ───────────────────────
ewm_topsis_viz(ewm_topsis, ALGO_NAMES, CAT_NAMES, CAT_COLORS, algo_cat, ...
    METRIC_NAMES, w_j, e_j, X_raw, R, V, D_pos, D_neg, C_score, ...
    rank_idx, rank_names, rank_cats, C_GREEN, C_GOLD, C_W, C_MID, cmap_gg);

fprintf('\nAll figures saved to current directory.\n');


%% ============================================================
%%  EWM-TOPSIS Visualization (Fig 8 + Fig 9)
%% ============================================================
function ewm_topsis_viz(ewm_topsis, ALGO_NAMES, CAT_NAMES, CAT_COLORS, algo_cat, ...
        METRIC_NAMES, w_j, e_j, X_raw, R, V, D_pos, D_neg, C_score, ...
        rank_idx, rank_names, rank_cats, C_GREEN, C_GOLD, C_W, C_MID, cmap_gg)
...
% 类别颜色辅助
cat_clr = @(a) CAT_COLORS{algo_cat(a)};

%% ── Fig 8: EWM权重 + TOPSIS排名综合图 (2×2布局) ─────────────
fig8 = figure('Color',C_W,'Name','fig8_ewm_topsis', ...
    'Position',[50 50 1600 900]);

% ── 8A (左上): EWM权重柱状图 ─────────────────────────────────
ax8a = subplot(2,3,1);
hold on; box on; grid on;
ax8a.GridColor = [0.88 0.88 0.85]; ax8a.GridAlpha = 0.5;
ax8a.Color = C_W;

bar_colors_w = zeros(nMetric, 3);
for j = 1:nMetric
    t = (j-1)/(nMetric-1);
    bar_colors_w(j,:) = (1-t)*C_GREEN + t*C_GOLD;
end
bh8a = bar(w_j, 'FaceColor','flat','EdgeColor','none','BarWidth',0.65);
for j=1:nMetric, bh8a.CData(j,:) = bar_colors_w(j,:); end

% 在柱子上显示权重数值
for j = 1:nMetric
    text(j, w_j(j)+max(w_j)*0.03, sprintf('%.3f', w_j(j)), ...
        'HorizontalAlignment','center','FontSize',9,'FontWeight','bold', ...
        'Color',[0.2 0.2 0.2]);
end

xticks(1:nMetric);
xticklabels(cellfun(@(s) strrep(s,'\_','_'), METRIC_NAMES, 'UniformOutput',false));
xtickangle(30);
ylabel('Objective Weight w_j','FontSize',10,'FontWeight','bold');
title('EWM — Objective Weights', ...
    'FontSize',11,'FontWeight','bold','Color',[0.2 0.2 0.2]);
set(ax8a,'TickDir','out','FontSize',9,'Box','off','XColor',[0.35 0.35 0.35]);

% ── 8B (右上): 信息熵 e_j 柱状图 ────────────────────────────
ax8b = subplot(2,3,2);
hold on; box on; grid on;
ax8b.GridColor = [0.88 0.88 0.85]; ax8b.GridAlpha = 0.5;
ax8b.Color = C_W;

bh8b = bar(e_j,'FaceColor','flat','EdgeColor','none','BarWidth',0.65);
for j=1:nMetric
    t = (j-1)/(nMetric-1);
    bh8b.CData(j,:) = (1-t)*[0.93 0.75 0.25] + t*C_GREEN;
end
yline(1/log(nAlgo),'--','Color',[0.5 0.5 0.5],'LineWidth',1.2);
for j=1:nMetric
    text(j, e_j(j)+0.008, sprintf('%.3f',e_j(j)), ...
        'HorizontalAlignment','center','FontSize',8.5,'Color',[0.25 0.25 0.25]);
end

xticks(1:nMetric);
xticklabels(cellfun(@(s) strrep(s,'\_','_'), METRIC_NAMES, 'UniformOutput',false));
xtickangle(30);
ylabel('Entropy e_j','FontSize',10,'FontWeight','bold');
title('EWM — Information Entropy e_j', ...
    'FontSize',11,'FontWeight','bold','Color',[0.2 0.2 0.2]);
set(ax8b,'TickDir','out','FontSize',9,'Box','off','XColor',[0.35 0.35 0.35]);

% ── 8C (右上角): 雷达图 — 每个类别最优算法的归一化指标分布 ─
ax8c = subplot(2,3,3);
% 用极坐标雷达图展示 EWM 权重分布
theta_m = linspace(0, 2*pi, nMetric+1); theta_m(end) = [];
metric_disp = cellfun(@(s) strrep(s,'\_','_'), METRIC_NAMES,'UniformOutput',false);

axes('Color',C_W,'Position',ax8c.Position); hold on;
% 参考圆
for rv=[0.25 0.5 0.75 1.0]
    th2=linspace(0,2*pi,200);
    plot(cos(th2)*rv,sin(th2)*rv,'-','Color',[0.87 0.87 0.85],'LineWidth',0.7);
end
for k=1:nMetric
    [xe_,ye_]=pol2cart(theta_m(k),1.18);
    plot([0 cos(theta_m(k))],[0 sin(theta_m(k))],'--','Color',[0.82 0.82 0.80],'LineWidth',0.8);
    text(xe_,ye_,metric_disp{k},'HorizontalAlignment','center', ...
        'FontSize',8.5,'Color',[0.25 0.25 0.25],'FontWeight','bold');
end

% 画权重雷达
w_loop = [w_j, w_j(1)]; th_loop = [theta_m, theta_m(1)];
[xp_,yp_] = pol2cart(th_loop, w_loop/max(w_j));
fill(xp_,yp_,C_MID,'FaceAlpha',0.22,'EdgeColor','none');
plot(xp_,yp_,'-o','Color',C_MID,'LineWidth',2.5,'MarkerSize',6, ...
    'MarkerFaceColor',C_MID,'MarkerEdgeColor','w');
for k=1:nMetric
    r_=w_j(k)/max(w_j);
    [xd_,yd_]=pol2cart(theta_m(k),r_*1.12);
    text(xd_,yd_,sprintf('%.3f',w_j(k)),'FontSize',8, ...
        'HorizontalAlignment','center','Color',C_MID*0.7,'FontWeight','bold');
end
axis equal off;
title('EWM Weight Radar','FontSize',11,'FontWeight','bold','Color',[0.2 0.2 0.2]);
delete(ax8c);

% ── 8D (左下): TOPSIS C_i 得分水平柱状图 ───────────────────
ax8d = subplot(2,3,[4,5]);
hold on; box on; grid on;
ax8d.GridColor = [0.88 0.88 0.85]; ax8d.GridAlpha = 0.4;
ax8d.Color = C_W;

bh8d = barh(C_sorted,'FaceColor','flat','EdgeColor','none','BarWidth',0.68);
for k = 1:nAlgo
    ai = rank_idx(k);
    bh8d.CData(k,:) = CAT_COLORS{algo_cat(ai)};
end

% 冠军标记
plot(C_sorted(1),1,'p','MarkerSize',15, ...
    'MarkerFaceColor',CAT_COLORS{algo_cat(rank_idx(1))}, ...
    'MarkerEdgeColor','w','LineWidth',0.8);

rng_c = max(C_sorted)-min(C_sorted); if rng_c<0.001, rng_c=0.001; end
for k=1:nAlgo
    ai = rank_idx(k);
    text(C_sorted(k)+rng_c*0.01, k, ...
        sprintf('#%d  %.4f', k, C_sorted(k)), ...
        'FontSize',8.5,'Color',[0.2 0.2 0.2],'VerticalAlignment','middle');
    % 类别标签
    text(min(C_sorted)-rng_c*0.02, k, ...
        sprintf('[%s]', CAT_NAMES{algo_cat(ai)}(1:3)), ...
        'FontSize',7.5,'Color',CAT_COLORS{algo_cat(ai)}*0.65, ...
        'HorizontalAlignment','right','VerticalAlignment','middle');
end

yticks(1:nAlgo); yticklabels(rank_names);
xlabel('TOPSIS Score C_i  (higher = better)','FontSize',10,'FontWeight','bold');
title(sprintf('TOPSIS Comprehensive Ranking — All 15 Algorithms  (Champion: %s)', ...
    rank_names{1}),'FontSize',11,'FontWeight','bold','Color',[0.15 0.15 0.15]);

% 类别图例
leg8d = gobjects(nCat,1);
for c=1:nCat, leg8d(c)=patch(NaN,NaN,CAT_COLORS{c},'EdgeColor','none'); end
legend(leg8d, CAT_NAMES,'Location','southeast','Box','off','FontSize',9);
set(ax8d,'TickDir','out','FontSize',9,'Box','off','XColor',[0.35 0.35 0.35]);

% ── 8E (右下): D+ vs D- 气泡散点图 ─────────────────────────
ax8e = subplot(2,3,6);
hold on; box on; grid on;
ax8e.GridColor = [0.88 0.88 0.85]; ax8e.GridAlpha = 0.4;
ax8e.Color = C_W;

for a=1:nAlgo
    clr = CAT_COLORS{algo_cat(a)};
    sz  = 80 + C_score(a)*300;    % 气泡大小反映 C_i
    scatter(D_pos(a), D_neg(a), sz, clr, 'filled', ...
        'MarkerEdgeColor','w','LineWidth',0.8,'MarkerFaceAlpha',0.85);
    text(D_pos(a), D_neg(a)+max(D_neg)*0.025, ALGO_NAMES{a}, ...
        'FontSize',8,'HorizontalAlignment','center', ...
        'Color',[0.2 0.2 0.2],'FontWeight','bold');
end

% 对角线参考: D+ = D- → C_i = 0.5
d_lim = max([D_pos;D_neg])*1.05;
plot([0 d_lim],[0 d_lim],'--','Color',[0.6 0.6 0.6],'LineWidth',1.2);
text(d_lim*0.55, d_lim*0.62,'C_i = 0.5','FontSize',8, ...
    'Color',[0.55 0.55 0.55],'Rotation',42);

xlabel('D^+  (Distance to Ideal)','FontSize',10,'FontWeight','bold');
ylabel('D^-  (Distance to Worst)','FontSize',10,'FontWeight','bold');
title('D^+ vs D^-  (bubble size ∝ C_i)', ...
    'FontSize',11,'FontWeight','bold','Color',[0.2 0.2 0.2]);
leg8e = gobjects(nCat,1);
for c=1:nCat, leg8e(c)=patch(NaN,NaN,CAT_COLORS{c},'EdgeColor','none'); end
legend(leg8e, CAT_NAMES,'Location','northwest','Box','off','FontSize',8.5);
set(ax8e,'TickDir','out','FontSize',9,'Box','off', ...
    'XColor',[0.35 0.35 0.35],'YColor',[0.35 0.35 0.35]);
xlim([0 d_lim]); ylim([0 d_lim]);

sgtitle('EWM-TOPSIS Multi-Criteria Evaluation', ...
    'FontSize',14,'FontWeight','bold','Color',[0.12 0.12 0.12]);
exportgraphics(fig8,'cat_fig8_ewm_topsis.png','Resolution',200);
fprintf('Saved: cat_fig8_ewm_topsis.png\n');

%% ── Fig 9: 归一化决策矩阵热力图 + 权重加权矩阵 ─────────────
fig9 = figure('Color',C_W,'Name','fig9_decision_matrix', ...
    'Position',[80 80 1400 780]);

% 9A 左: 归一化矩阵 R
ax9a = subplot(1,2,1);
imagesc(R');
colormap(ax9a, cmap_gg);
try, clim([0 1]); catch, caxis([0 1]); end
cb9a = colorbar('southoutside');
cb9a.Label.String = 'Normalized Score r_{ij}  (0→worst, 1→best)';
cb9a.FontSize = 8;

for i=1:nAlgo
    for j=1:nMetric
        text(i,j,sprintf('%.2f',R(i,j)), ...
            'HorizontalAlignment','center','VerticalAlignment','middle', ...
            'FontSize',7.5,'Color',[0.1 0.1 0.1],'FontWeight','bold');
    end
end

xticks(1:nAlgo); xticklabels(ALGO_NAMES); xtickangle(40);
yticks(1:nMetric);
yticklabels(cellfun(@(s) strrep(s,'\_','_'), METRIC_NAMES,'UniformOutput',false));

% 给每个算法x轴标签着色
ax9a.XAxis.TickLabelColor = [0.2 0.2 0.2];

xlabel('Algorithm','FontSize',10,'FontWeight','bold');
title('Normalized Decision Matrix R  (r_{ij})', ...
    'FontSize',11,'FontWeight','bold','Color',[0.2 0.2 0.2]);
set(ax9a,'TickDir','out','FontSize',8.5,'Box','off');

% 9B 右: 加权矩阵 V，并叠加 C_i 排名条
ax9b = subplot(1,2,2);
imagesc(V');
colormap(ax9b, cmap_gg);
mx_v = max(V(:)); if mx_v<1e-10, mx_v=1; end
try, clim([0 mx_v]); catch, caxis([0 mx_v]); end
cb9b = colorbar('southoutside');
cb9b.Label.String = 'Weighted Score v_{ij} = w_j · r_{ij}';
cb9b.FontSize = 8;

for i=1:nAlgo
    for j=1:nMetric
        text(i,j,sprintf('%.3f',V(i,j)), ...
            'HorizontalAlignment','center','VerticalAlignment','middle', ...
            'FontSize',7.5,'Color',[0.1 0.1 0.1],'FontWeight','bold');
    end
end
...
% 添加每列标题行注释 (权重)
for j=1:nMetric
    text(nAlgo+0.55, j, sprintf('w=%.3f',w_j(j)), ...
        'FontSize',7.5,'Color',C_MID*0.7,'VerticalAlignment','middle', ...
        'HorizontalAlignment','left');
end

xlabel('Algorithm','FontSize',10,'FontWeight','bold');
title('Weighted Decision Matrix V  (v_{ij} = w_j · r_{ij})', ...
    'FontSize',11,'FontWeight','bold','Color',[0.2 0.2 0.2]);
set(ax9b,'TickDir','out','FontSize',8.5,'Box','off');

sgtitle('EWM-TOPSIS Decision Matrices', ...
    'FontSize',13,'FontWeight','bold','Color',[0.12 0.12 0.12]);
exportgraphics(fig9,'cat_fig9_decision_matrices.png','Resolution',200);
fprintf('Saved: cat_fig9_decision_matrices.png\n');

end  % ewm_topsis_viz


%% ============================================================
%%  Original Visualization — Categorized Benchmark (Figs 1-7)
%% ============================================================
...
bw = 0.35;
w_idx = cat_winner_idx;
for c=1:nCat
    ai = w_idx(c);
    bar(c-bw/2, mean_overall(ai), bw, 'FaceColor',CAT_COLORS{c}, ...
        'EdgeColor','none','FaceAlpha',0.90);
    bar(c+bw/2, std_overall(ai),  bw, 'FaceColor',CAT_COLORS{c}*0.55+[1 1 1]*0.45, ...
        'EdgeColor','none','FaceAlpha',0.90);
...
%% ── Fig 3: Full 15-Algorithm Ranking ──
...
%% ── Fig 5: Per-Category SHAP Beeswarm ──
...
%% ── Fig 6: Pairwise Interaction Matrix ──
