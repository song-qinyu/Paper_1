...
lightn = zeros(n, 1);
for i = 1:n
    lightn(i) = fa_obj(zn(i,:), data, weights);
end
Convergence_curve = zeros(1, MaxGeneration);

% ========================== 3. 执行 FA 优化 ==========================
fprintf('FA 优化启动，正在生成统一规格图表...\n');
    ...
    for i = 1:n
        for j = 1:n
            if lightn(i) > lightn(j)
                ...
                zn(i,:) = zn(i,:) * (1 - beta) + zn(j,:) * beta + alpha * (rand(1, dim) - 0.5);
                zn(i,:) = max(min(zn(i,:), ub), lb);
                lightn(i) = fa_obj(zn(i,:), data, weights);
            end
        end
    end
    [fmin, best_idx] = min(lightn);
    bestX = zn(best_idx, :);
    Convergence_curve(k) = fmin;
end
...
figure('Color','w', 'Name', 'FA 2D Allocation', 'Position', [100, 100, 850, 850]); 
hold on; box on;
...
title('FA Optimized Shelter Allocation Map', 'FontSize', 14, 'FontWeight', 'bold');
...
% --- 图 2: 收敛曲线 ---
figure('Color','w', 'Name', 'FA Convergence', 'Position', [200, 200, 600, 450]);
...
title('FA Optimization Convergence Process', 'FontSize', 12);

% --- 图 3: 3D 需求压力地形图 ---
figure('Color','w', 'Name', 'FA 3D Landscape', 'Position', [150, 150, 900, 750]);
hold on; grid on;
...
colormap(jet); shading interp; cb = colorbar; ylabel(cb, 'Demand Intensity');
...
title('FA 3D Demand Landscape & Shelter Location', 'FontSize', 12);

fprintf('FA 任务完成：图表已对齐其他算法规格。\n'); toc;
