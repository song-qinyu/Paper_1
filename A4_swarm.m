%% 四算法疏散分配对比 - 一键运行版
%  包含 PSO / GWO / SSA 三种算法 + ACO（需 standard.m 在同目录）
%  以 PSO 为底图基准，叠加其余三算法的差异分配线
%
%  输出图:
%    图1  PSO 收敛曲线
%    图2  GWO 收敛曲线
...
%    图4  四算法叠加对比地图（核心输出）
%
%  注意: ACO 计算量极大，默认关闭，如需开启见 Section 4
%
...
diff_ACO = aco_available & (eID_ACO ~= eID_PSO);
...
if aco_available
    fprintf('ACO 与 PSO 不同      : %d 条 (%.1f%%)\n', sum(diff_ACO), 100*mean(diff_ACO));
end
...
plot(gwo_curve,'LineWidth',2,'Color',[0.42,0.78,0.60]);
grid on; box on;
xlabel('Iteration','FontWeight','bold');
ylabel('Best Fitness','FontWeight','bold');
...
plot(ssa_curve,'LineWidth',2,'Color',[0.93,0.76,0.10]);
grid on; box on;
xlabel('Iteration','FontWeight','bold');
ylabel('Best Fitness','FontWeight','bold');
...
for i = find(diff_GWO)
    hIdx = data.DFenPei{i}(1);
    h = line([house_x(hIdx), binan_x(eID_GWO(i))], ...
             [house_y(hIdx), binan_y(eID_GWO(i))], ...
             'Color',[0.42,0.78,0.60,0.65],'LineWidth',1.0);
    if isempty(h_gwo_diff), h_gwo_diff = h; end
end
...
for i = find(diff_SSA)
    hIdx = data.DFenPei{i}(1);
    h = line([house_x(hIdx), binan_x(eID_SSA(i))], ...
             [house_y(hIdx), binan_y(eID_SSA(i))], ...
             'Color',[0.93,0.76,0.10,0.65],'LineWidth',1.0);
    if isempty(h_ssa_diff), h_ssa_diff = h; end
end
...
title('PSO / ACO / GWO / SSA Allocation Comparison Map', ...
...
h_gwo_leg = line(NaN,NaN,'Color',[0.42,0.78,0.60,0.80],'LineWidth',2.0);
h_ssa_leg = line(NaN,NaN,'Color',[0.93,0.76,0.10,0.90],'LineWidth',2.0);

legend([h_shl, h_res, h_pso_leg, h_aco_leg, h_gwo_leg, h_ssa_leg], ...
    {'Shelter', ...
     'Residential', ...
     'PSO allocation', ...
...
fprintf('  春日青线   = GWO 与 PSO 不同的分配连线\n');
fprintf('  黄色线     = SSA 与 PSO 不同的分配连线\n');
fprintf('  绿色三角   = 避难场所\n');
fprintf('  蓝色圆点   = 住宅点位\n');
