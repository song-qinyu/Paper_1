...
% ========================== 2. WDO 参数设置 ==========================
popsize = 30;           % 种群规模
npar = length(data.DFenPei); % 维度
maxit = 100;            % 最大迭代次数
RT = 3; g = 0.2; alp = 0.4; c = 0.4; maxV = 0.3; % WDO 核心系数
weights.w1 = 0.001; weights.w2 = 1.0;

% 初始化
pos = rand(popsize, npar);      % 位置
vel = maxV * (2*rand(popsize, npar) - 1); % 速度
fit = zeros(popsize, 1);
for i = 1:popsize
    fit(i) = wdo_obj(pos(i,:), data, weights);
end
[globalMin, gbest_idx] = min(fit);
globalBest = pos(gbest_idx, :);
Convergence_curve = zeros(1, maxit);

% ========================== 3. 执行 WDO 优化 ==========================
fprintf('WDO 优化启动，正在生成统一规格图表...\n');
for it = 1:maxit
    for i = 1:popsize
        % 更新速度 (WDO 核心公式)
        r = rand(1, npar);
        vel(i,:) = (1-alp)*vel(i,:) - (g*pos(i,:)) + ...
                   abs(1-1/i)*((globalBest - pos(i,:)).*RT.*r) + ...
                   (c*vel(randi(popsize),:)/i);
    ...
    [currentMin, cbest_idx] = min(fit);
    if currentMin < globalMin
        globalMin = currentMin;
        globalBest = pos(cbest_idx, :);
    end
    Convergence_curve(it) = globalMin;
end
...
% --- 图 1: 2D 空间分配地图 (占比最大化) ---
figure('Color','w', 'Name', 'WDO 2D Allocation', 'Position', [100, 100, 850, 850]); 
hold on; box on;
...
title('WDO Optimized Shelter Allocation Map', 'FontSize', 14, 'FontWeight', 'bold');
...
% --- 图 2: 收敛曲线 ---
figure('Color','w', 'Name', 'WDO Convergence', 'Position', [200, 200, 600, 450]);
plot(Convergence_curve, 'LineWidth', 2.5, 'Color', [0.85, 0.33, 0.1]); 
grid on; box on;
xlabel('Iteration'); ylabel('Best Fitness');
title('WDO Optimization Convergence Process', 'FontSize', 12);

% --- 图 3: 3D 需求压力地形图 ---
figure('Color','w', 'Name', 'WDO 3D Landscape', 'Position', [150, 150, 900, 750]);
hold on; grid on;
...
title('WDO 3D Demand Landscape & Shelter Location', 'FontSize', 12);

fprintf('WDO 任务完成：图表规格已统一。\n'); toc;
