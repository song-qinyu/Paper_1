function [R_best,F_best,L_best,T_best,L_ave,Shortest_Route,Shortest_Length] = ant_19(D,initial,destination,dis,NC_max,m,MM,Lgrid)
%% 文献19
...
%计算待访问栅格的转移概率分布和启发式信息概率分布
            for k=1:length(J)           %sum(J>0)表示待访问的栅格的个数
                Eta = [];
                Eta = 1.25/(D(visited(end),N(k))+dis(J(k)));%%%%%%%%对应文献中1.2.1节
                Pz(k)=(Tau(visited(end),N(k))^Alpha)*(Eta^Beta);  %传统概率计算公式中的分子
                Phi(k)=((max_dis-dis(J(k)))*Omega+Mu)^Lambda;%原本改进的东西
%                 Phi(k)=((max_dis-dis(J(k)))+1)^1;
            end
            Pz=Pz/(sum(Pz));               %传统转移概率分布：长度为待访问栅格个数
            Phi=Phi/(sum(Phi));         %启发式信息概率分布，改进的
%             P = Pz*0.7+Phi*0.3;%两种概率加权平均，改进的
            P = Pz;
%% 改进转移概率，轮盘赌
            %按概率原则选取下一个栅格
