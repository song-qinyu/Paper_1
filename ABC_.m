...
% 初始化
Foods = lb + rand(FoodNumber, dim) * (ub - lb);
for i = 1:FoodNumber
    ObjVal(i) = abc_obj(Foods(i,:), data, weights);
end
Fitness = 1./(1 + ObjVal); % 计算适应度
trial = zeros(1, FoodNumber);
Convergence_curve = zeros(1, MaxCycle);

% ========================== 3. 执行 ABC 优化 ==========================
fprintf('ABC 优化启动，正在生成统一规格图表...\n');
for iter = 1:MaxCycle
    % 雇佣蜂阶段
    for i = 1:FoodNumber
        k = floor(rand * FoodNumber) + 1;
        while k == i, k = floor(rand * FoodNumber) + 1; end
        phi = (rand(1, dim) * 2 - 1);
        newFood = Foods(i,:) + phi .* (Foods(i,:) - Foods(k,:));
        newFood = max(min(newFood, ub), lb);
        newObj = abc_obj(newFood, data, weights);
        if newObj < ObjVal(i)
            Foods(i,:) = newFood; ObjVal(i) = newObj; trial(i) = 0;
        else
            trial(i) = trial(i) + 1;
        end
    end
    ...
    % 侦查蜂阶段
    [maxTrial, idx] = max(trial);
    if maxTrial > limit
        Foods(idx,:) = lb + rand(1, dim) * (ub - lb);
        ObjVal(idx) = abc_obj(Foods(idx,:), data, weights);
        trial(idx) = 0;
    end
    
    Fitness = 1./(1 + ObjVal);
    [fmin, best_idx] = min(ObjVal);
    Convergence_curve(iter) = fmin;
end
bestX = Foods(best_idx, :);
...
% --- 图 1: 2D 空间分配地图 (占比最大化) ---
figure('Color','w', 'Name', 'ABC 2D Allocation', 'Position', [100, 100, 850, 850]); 
hold on; box on;
...
title('ABC Optimized Shelter Allocation Map', 'FontSize', 14, 'FontWeight', 'bold');
...
% --- 图 2: 收敛曲线 ---
figure('Color','w', 'Name', 'ABC Convergence', 'Position', [200, 200, 600, 450]);
plot(Convergence_curve, 'LineWidth', 2.5, 'Color', [0.85, 0.33, 0.1]); 
grid on; box on;
xlabel('Iteration', 'FontWeight', 'bold'); ylabel('Best Fitness', 'FontWeight', 'bold');
title('ABC Optimization Convergence Process', 'FontSize', 12);

% --- 图 3: 3D 需求压力地形图 ---
figure('Color','w', 'Name', 'ABC 3D Landscape', 'Position', [150, 150, 900, 750]);
hold on; grid on;
...
title('ABC 3D Demand Landscape & Shelter Location', 'FontSize', 12);

fprintf('ABC 任务完成：图表已统一对齐。\n'); toc;
