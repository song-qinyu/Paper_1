...
child_node1 = [parent_node(1), parent_node(2)+1];
child_node2 = [parent_node(1)-1, parent_node(2)];
if inpolygon(child_node(1), child_node(2), field(:,1), field(:,2))%判断点是否在多边形内
    if ~ismember(child_node, obs, 'rows')&&(~ismember(child_node1, obs, 'rows')||~ismember(child_node2, obs, 'rows'))
...
% 第5个子节点
child_node = [parent_node(1)+1, parent_node(2)-1];
child_node1 = [parent_node(1)+1, parent_node(2)];
child_node2 = [parent_node(1), parent_node(2)-1];
if inpolygon(child_node(1), child_node(2), field(:,1), field(:,2))
    if ~ismember(child_node, obs, 'rows')&&(~ismember(child_node1, obs, 'rows')||~ismember(child_node2, obs, 'rows'))
        child_nodes = [child_nodes; child_node];
    end
