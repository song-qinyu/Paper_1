...
plot(de_curve,'LineWidth',2,'Color',[0.20,0.60,0.80]);
grid on; box on;
xlabel('Iteration','FontWeight','bold');
ylabel('Best Fitness','FontWeight','bold');
...
plot(nsga_fSol(:,1), nsga_fSol(:,2), 'o', ...
     'MarkerEdgeColor',[0.85,0.33,0.10], ...
     'MarkerFaceColor',[1.00,0.90,0.80], ...
...
% --- 图例 ---
h_ga_leg   = line(NaN,NaN,'Color',[0.90,0.10,0.10,0.80],'LineWidth',1.8);
h_de_leg   = line(NaN,NaN,'Color',[0.55,0.10,0.90,0.50],'LineWidth',1.8);
h_nsga_leg = line(NaN,NaN,'Color',[0.42,0.78,0.60,0.80],'LineWidth',1.8);

legend_entries = {h_shl, h_res, h_ga_leg};
legend_labels  = {'Shelter', 'Residential', 'GA allocation (base)'};

if ~isempty(h_de_diff)
    legend_entries{end+1} = h_de_leg;
    legend_labels{end+1}  = sprintf('DE differs from GA (n=%d)', sum(diff_DE));
end
if ~isempty(h_nsga_diff)
    legend_entries{end+1} = h_nsga_leg;
    legend_labels{end+1}  = sprintf('NSGA-II differs from GA (n=%d)', sum(diff_NSGA));
end

legend(legend_entries, legend_labels, 'Location','northeast','FontSize',10);
