%% ============================================================
%  Pipeline Stage 竞选框架 v3 — 全15算法 + SHAP风格全套可视化
%  绿金双色系 | beeswarm | Lowess | 热力图 | 雷达图
%  直接运行即可，跑完自动出7张图并保存PNG
%% ============================================================
clc; clear; close all; tic;

fprintf('========================================================\n');
fprintf('  Pipeline Stage 竞选框架 v3 — 全15算法公平评测\n');
fprintf('========================================================\n\n');

%% ==================== 0. 数据加载 ====================
if exist('sj5.mat','file'), load('sj5.mat');
else, error('未找到 sj5.mat，请将数据文件放至同一目录'); end
...
%% ==================== 1. 参数设置 ====================
NFE_per_algo = 15000;   % 每阶段每算法函数评价次数（调试用5000，正式用20000+）
K_elite      = 20;      % 精英池大小
...
%% ==================== 4. 构建数据矩阵 ====================
% all_fit: nAlgo × nStage，需处理各阶段算法名可能不一致的情况
...
set_ax = @(ax) set(ax,'Color',C_W,'Box','off','TickDir','out','FontSize',9,...
    'GridAlpha',0.12,'GridColor',[0.6 0.6 0.6],'XColor',[0.35 0.35 0.35],'YColor',[0.35 0.35 0.35]);

%% ----- 图1：各阶段排名条形图 + 收敛趋势 -----
fig1 = figure('Color',C_W,'Name','fig1_ranking','Position',[30 30 1600 900]);
...
for s = 1:nStage
    ...
    names_s = algos(si);
    n = length(fits_s);
    f_norm = (fits_s-min(fits_s)) / (max(fits_s)-min(fits_s)+1e-10);
    colors = zeros(n,3);
    for i=1:n
    ...
    hold on;
    % 第1名五角星
    plot(fits_s(1), 1, 'p','MarkerFaceColor',C_GREEN,'MarkerEdgeColor','w',...
        'MarkerSize',11,'LineWidth',0.5);
    % 数值标注
    for i=1:n
        text(fits_s(i)+range(fits_s)*0.005, i, ...
            sprintf('%.0f',fits_s(i)), 'FontSize',7.5,'Color',[0.3 0.3 0.3],...
            'VerticalAlignment','middle');
    end
    yticks(1:n); yticklabels(names_s);
    xlabel('总距离 (m)','FontSize',9);
...
x6 = 1:nStage;
fill([x6,fliplr(x6)],[stage_bests+range(stage_bests)*0.05,...
    fliplr(stage_bests-range(stage_bests)*0.05)],...
    C_LG,'EdgeColor','none','FaceAlpha',0.35); hold on;
plot(x6,stage_bests,'-o','LineWidth',2.5,'Color',C_GREEN,...
    'MarkerFaceColor',C_GREEN,'MarkerSize',8,'MarkerEdgeColor','w');
for s=1:nStage
    text(s, stage_bests(s)+range(stage_bests)*0.06, stage_winners{s},...
        'HorizontalAlignment','center','FontSize',9,...
        'Color',C_GREEN,'FontWeight','bold');
end
xlim([0.6 nStage+0.4]); xticks(1:nStage);
xticklabels(cellfun(@(l,k) sprintf('S%d\n%s',k,l), stage_labels, num2cell(1:nStage),'UniformOutput',false));
ylabel('最优适应度 (m)','FontSize',9);
title('Pipeline 收敛趋势','FontSize',11,'FontWeight','bold','Color',[0.2 0.2 0.2]);
set_ax(ax6); grid on;

sgtitle('各阶段算法竞选排名（总距离越短越优）',...
    'FontSize',13,'FontWeight','bold','Color',[0.15 0.15 0.15]);
exportgraphics(fig1,'fig1_stage_ranking.png','Resolution',200);
fprintf('✓ 已保存 fig1_stage_ranking.png\n');

%% ----- 图2：SHAP 全局特征贡献图（beeswarm风格）-----
% SHAP贡献 = (阶段均值 - 算法适应度) / 阶段均值
...
fig2 = figure('Color',C_W,'Name','fig2_beeswarm','Position',[60 60 920 640]);
ax2 = axes('Color',C_W); hold on;

for i = 1:nAlgo
    y_vals = shap_s(i,:);
    % 均值条（半透明）
    barh(i, mean_shap_s(i), 0.4,'FaceColor',C_LG.*0.7+C_W*0.3,...
    ...
    for j=1:nStage
        ...
        scatter(abs(y_vals(j)), yj, 45, c,'filled','MarkerFaceAlpha',0.82,'MarkerEdgeColor','none');
    end
    % 百分比
    pct = mean_shap_s(i)/sum(mean_shap_s)*100;
    text(mean_shap_s(i)+max(mean_shap_s)*0.01, i,...
        sprintf(' %.1f%%',pct),'FontSize',8,'Color',[0.3 0.3 0.3],'VerticalAlignment','middle');
end

yticks(1:nAlgo); yticklabels(algos_s);
xlabel('Mean |SHAP| (相对贡献值)','FontSize',10);
title('SHAP 全局算法贡献图','FontSize',12,'FontWeight','bold','Color',[0.2 0.2 0.2]);
...
cb2.Label.String = '阶段适应度  Low → High';
cb2.Ticks=[0,1]; cb2.TickLabels={'差(负贡献)','优(正贡献)'}; cb2.FontSize=8;

set_ax(ax2); grid on; ax2.XGrid='on'; ax2.YGrid='off';
exportgraphics(fig2,'fig2_shap_beeswarm.png','Resolution',200);
fprintf('✓ 已保存 fig2_shap_beeswarm.png\n');
...
fig3 = figure('Color',C_W,'Name','fig3_main_interact','Position',[90 50 1150 430]);
ax3 = axes('Color',C_W); hold on;

bw3 = 0.36;
bm  = bar((1:nAlgo)-bw3/2, main_eff(si3),  bw3,'FaceColor',C_GREEN,'EdgeColor','none','FaceAlpha',0.88);
bi3 = bar((1:nAlgo)+bw3/2, inter_eff(si3), bw3,'FaceColor',C_GOLD, 'EdgeColor','none','FaceAlpha',0.88);

for k=1:nAlgo
    text(k-bw3/2, main_eff(si3(k))+max(main_eff)*0.015,...
        sprintf('%.3f',main_eff(si3(k))),'FontSize',7,...
        'HorizontalAlignment','center','Color',[0.2 0.2 0.2]);
    text(k+bw3/2, inter_eff(si3(k))+max(inter_eff)*0.015,...
...
for i=1:nAlgo
    for j=1:nStage
        v=hmap(i,j);
        fc = abs(v)/clim_v;
        txt_c = [0.15 0.15 0.15];
        text(j,i,sprintf('%.3f',v),'HorizontalAlignment','center',...
            'VerticalAlignment','middle','FontSize',8.5,'Color',txt_c,'FontWeight','bold');
    end
end

xticks(1:nStage);
xticklabels(cellfun(@(l,k) sprintf('S%d %s',k,l), stage_labels,...
    num2cell(1:nStage),'UniformOutput',false));
yticks(1:nAlgo); yticklabels(algos(row_order));
xlabel('Pipeline 阶段','FontSize',10);
title('SHAP 热力图（算法 × 阶段贡献矩阵）','FontSize',12,'FontWeight','bold','Color',[0.2 0.2 0.2]);
set(ax4,'TickDir','out','FontSize',9,'Box','off');

% 顶部 f(x) 折线（参考图8顶部黑色曲线）
ax4t = axes('Position',[0.12 0.845 0.72 0.10],'Color',C_W);
stage_bests2 = min(all_fit,[],1);
patch([1:nStage,fliplr(1:nStage)],...
    [stage_bests2+range(stage_bests2)*0.08,...
     fliplr(stage_bests2-range(stage_bests2)*0.08)],...
    [0.75 0.75 0.72],'EdgeColor','none','FaceAlpha',0.4); hold on;
plot(1:nStage, stage_bests2,'k-','LineWidth',1.5);
scatter(1:nStage, stage_bests2,30,'k','filled');
xlim([0.5 nStage+0.5]); axis off;
text(0.3, mean(stage_bests2),'f(x)','FontSize',9,'Color',[0.3 0.3 0.3],'FontStyle','italic');

exportgraphics(fig4,'fig4_shap_heatmap.png','Resolution',200);
fprintf('✓ 已保存 fig4_shap_heatmap.png\n');
...
fig5 = figure('Color',C_W,'Name','fig5_cross_matrix','Position',[150 30 1050 900]);
ax5 = axes('Color',C_W);
imagesc(cross_mat);
colormap(ax5, cmap_gg);
mx5=max(abs(cross_mat(:))); caxis([-mx5 mx5]);
cb5=colorbar; cb5.Label.String='性能差 (m，正=行算法优于列算法)'; cb5.FontSize=8;
    ...
    for j=1:nAlgo
        ...
        if i~=j && abs(v)>mx5*0.15
            text(j,i,sprintf('%.0f',v),'HorizontalAlignment','center',...
                'VerticalAlignment','middle','FontSize',7,'Color',[0.15 0.15 0.15]);
        end
    end
end
...
xlabel('对比算法（列）','FontSize',10); ylabel('参考算法（行）','FontSize',10);
title('算法性能交互矩阵','FontSize',12,'FontWeight','bold','Color',[0.2 0.2 0.2]);
set(ax5,'TickDir','out','FontSize',9,'Box','off');
exportgraphics(fig5,'fig5_algo_interaction_matrix.png','Resolution',200);
fprintf('✓ 已保存 fig5_algo_interaction_matrix.png\n');

%% ----- 图6：SHAP 依赖图（参考图3 Lowess风格，前6算法）-----
[~,top6_idx] = sort(mean(abs(shap_mat),2),'descend');
top6 = top6_idx(1:min(6,nAlgo));

fig6 = figure('Color',C_W,'Name','fig6_dependence','Position',[80 60 1300 820]);
for k=1:length(top6)
    ax = subplot(2,3,k);
    a_idx = top6(k);
    ...
    yl_v = max(abs(y_vals))*1.3+0.001;
    xl_v = [min(x_vals)-range(x_vals)*0.08, max(x_vals)+range(x_vals)*0.08];
    ...
    % 散点（颜色=阶段编号）
    scatter_c = interp1(linspace(0,1,nStage),...
        [C_GREEN;(C_GREEN+C_GOLD)/2;C_GOLD;C_GOLD*0.8+C_LY*0.2;C_GOLD*0.6+C_LY*0.4],...
        linspace(0,1,nStage));
    for j=1:nStage
        xj = x_vals(j)+(rand-0.5)*range(x_vals)*0.03;
        scatter(xj, y_vals(j), 65, scatter_c(min(j,size(scatter_c,1)),:),...
            'filled','MarkerFaceAlpha',0.85,'MarkerEdgeColor','w','LineWidth',0.4);
    end
    ...
    if nStage>=4
        [xs,si_] = sort(x_vals);
        try
            ys = smooth(x_vals(:), y_vals(:), 0.8, 'lowess');
            plot(xs, ys(si_), '-','LineWidth',2.2,'Color',C_GREEN);
        catch
            p=polyfit(x_vals,y_vals,1);
            plot(xl_v, polyval(p,xl_v),'-','LineWidth',2.2,'Color',C_GREEN);
        end
    else
        p=polyfit(x_vals,y_vals,1);
    ...
    xlabel('适应度 (m)','FontSize',9);
    ylabel('SHAP value','FontSize',9);
    title(algos{a_idx},'FontSize',11,'FontWeight','bold','Color',[0.2 0.2 0.2]);
    set_ax(ax); grid on; ax.GridAlpha=0.08;
end
sgtitle('SHAP 单算法依赖图（Lowess拟合）',...
    'FontSize',12,'FontWeight','bold','Color',[0.15 0.15 0.15]);
exportgraphics(fig6,'fig6_shap_dependence.png','Resolution',200);
fprintf('✓ 已保存 fig6_shap_dependence.png\n');

%% ----- 图7：雷达图（Top5算法，各阶段综合得分）-----
...
fig7 = figure('Color',C_W,'Name','fig7_radar','Position',[200 80 860 720]);
ax7  = axes('Color',C_W); hold on;
...
legend(algos(top5),'Location','southoutside','Orientation','horizontal',...
    'FontSize',9.5,'Box','off','TextColor',[0.25 0.25 0.25]);
axis equal off;
title('Top 5 算法·阶段综合得分雷达图',...
    'FontSize',12,'FontWeight','bold','Color',[0.2 0.2 0.2]);

exportgraphics(fig7,'fig7_radar.png','Resolution',200);
fprintf('✓ 已保存 fig7_radar.png\n');

fprintf('\n全部7张图已保存至当前目录。\n');
end  % function pipeline_viz
