...
function [Best_score,Best_pos,curve]=GA(pop,Max_iter,lb,ub,dim,fobj,G)
%% 参数初始化
popsize=pop;              %种群规模
lenchrom=dim;              %变量字串长度
fun = fobj;  %适应度函数
...
for i=1:popsize
    %随机产生一个种群
    GApop(i,:)=Code(lenchrom,bound,G);       %随机产生个体
    %计算适应度
...
%         disp(['第',num2str(i),'次迭代'])
    ...
    zbest = LocalSearch(zbest,ub(1),G);%%%%%把全局最优解进行局部搜索，提高全局最优解适应度值
    fitnesszbest = fun(zbest);         
...
%% 编码函数
function ret=Code(lenchrom,bound,G)
...
while flag==0
    ...
    for j=1:lenchrom
       ...
       ret(1,j) =  id(randi(length(id))); % 随机选择一个自由栅格
    end
